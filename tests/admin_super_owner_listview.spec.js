...
describe('Admin Super Owner API', () => {
  test('List admin super owner via API', async ({ request }) => {
    // Get auth token using helper
    const token = await getAuthToken(request);

    // Make the listview_partner API call
    const response = await request.post(`${BASE_URL}/listview_super_owner`, {
      headers: {
    ...
    await test.info().attach('List Admin Super Owner Response', {
      body: typeof responseBody === 'string' ? responseBody : JSON.stringify(responseBody, null, 2),
      contentType: contentType && contentType.includes('application/json') ? 'application/json' : 'text/plain',
    });
  });
});
